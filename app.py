...
def login():
    if request.form.get('password') == ADMIN_PASSWORD:
        # session.permanent を設定しないことで、ブラウザ終了時にセッションが消える設定になります
...
def index():
    ...
    if sel_platform != 'All':
        filtered = [r for r in filtered if str(r.get('platform')).strip() == sel_platform or str(r.get('platform')).strip() == 'Unknown']
    if sel_style != 'All':
